...
public:
    ...
    Board::Board()
    {
         ...
         chances[0].setup(4, 2, "Chance", "C1");
         chances[1].setup(8, 4, "Chance", "C2");
         chances[2].setup(6, 8, "Chance", "C3");
         chances[3].setup(2, 6, "Chance", "C4");
    }
	...
	int Board::doLand(int pn)
	{
	    for (int i=0; i<maxPlayers * 2; i++)
        {
                ...
                // if (pn != i || pn != (i+4) || pn != (i-4)) this will not move your own piece back home of you land on it
            {
                ...
                {
                    gotoxy(10, 55);
                    cout<<"You land on another player's piece.";
                            players[i].goHome();
