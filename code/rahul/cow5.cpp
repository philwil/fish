...
Turn links into a list  so we can have more than 4 for other projects
IF we like a link and it is not in the list then we can add a link pad combo
...
class cowLink {
public:
  cowLink(cowPad *a, int x , int y, int pad=0) {
        level=0;
        prev = a;
        next = NULL;
        dist = 0.0;
...
// this is a place we could put a cow.
// it will have x and y coordinates

// we'll also give it some link options.
// for now we can have N S E W links each of which will
// allow us to calculate the distance to the barn for each link.
// we pick the best one and allocate the link and its associated cowppad.
...
int  eval_cowpad(cowPad *cp) {
    ...
    // todo check for rocks val is -ve for a rock
    ...
    // todo check for 0 which means you have arrived
    maxd = n;
    if (s > maxd) maxd = s;
    if (e > maxd) maxd = e;
    if (w > maxd) maxd = w;
    ...
    if((n < 0) || (cp->N)) n = maxd+1;
    if((e < 0) || (cp->E)) e = maxd+1;
    if((w < 0) || (cp->W)) w = maxd+1;
    if((s < 0) || (cp->S)) s = maxd+1;
    best = "None";
    // simple test
    if ((cp->W == NULL) && (w<=n) && (w <= s) && (w <= e) ) {
        best = "West";
        cout << "adding cowpad " << best<<endl;
        cp->W = new cowLink(cp, -1, 0, 1);
        //cp->W->addCowPad(cp);
    }
    else if ((cp->E == NULL) && (e<=n) && (e <= s) && (e <= w) ) {
        best = "East";
        cout << "adding cowpad " << best<<endl;
        cp->E = new cowLink(cp, 1, 0, 1);
        //cp->E->addCowPad(cp);
    }
    else if ((cp->S == NULL) && (s<=n) && (s <=e) && (s <= w) ) {
        best = "South";
        cout << "adding cowpad " << best<<endl;
        cp->S = new cowLink(cp, 0, 1, 1);
        //cp->S->addCowPad(cp);
