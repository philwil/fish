...
while (word[idx] != ')')
 ...
 idx++;
    ...
    void KNode::print (KNode *indata, int option)
    {
        ...
        {
            pleft++;
            print(indata->getLeft(), option); // This is a recursion because the function is calling itself
            // print  // getLeft, so we have a new root
        }
        if
        (
        (option == 1) ||
        ((option == 2) && (indata->getcoordInCharge() == 'X')) ||
        ((option == 4) && (indata->getcoordInCharge() == 'Y')) ||
        ((option == 3) && (indata->getLeft() == NULL || indata->getRight() == NULL))
        )
        {
        ...
        {
            pright++;
            print(indata->getRight(), option); // This is a recursion because the function is calling itself
