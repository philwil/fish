...
public:
    ...
    string newstr;
    int compare (BinaryNum); // Returns 0 if same,  returns (-1) if less, returns (+1) if greater
};
...
int BinaryNum::compare (BinaryNum other)
{
    int i;
    int value = 0;
    for (i = 0; i < stringVal.length(); i++)
    {
        if ((stringVal[i] == '1') && (other.stringVal[i] == '0'))
        {
            return 1;
        }
        if ((stringVal[i] == '0') && (other.stringVal[i] == '1'))
        {
            return -1;
        }
    }
...
int main(){
    ...
    int value = firstBin.compare(secondBin);
    if (value == 0)
    {
