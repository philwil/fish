...
int run_non_pointer(void)
{
  vector <MyFish> mfv;
  cout << " NON Pointer At Start " << endl; 
  mfv.push_back(MyFish("herman"));
  mfv.push_back(MyFish("pupkin"));
  mfv.push_back(MyFish("kiwi"));
  cout << " NON Pointer All Done " << endl; 
}

int cleanup_pointer (vector <MyFish*> *mfv)
{
  vector <MyFish*> ::iterator mfi;
  for (mfi = mfv->begin(); mfi != mfv->end();++mfi )
    {
      //cout << "removing ["<< *mfi<<"]"<< endl; 
      delete *mfi;
    }
  return 0;
}
int run_pointer(void)
{
  MyFish * hf;
  vector <MyFish*> mfv;
  cout << " Pointer At Start " << endl; 
  mfv.push_back(new MyFish("herman"));
  mfv.push_back(new MyFish("pupkin"));
  mfv.push_back(new MyFish("kiwi"));
  cout << " Pointer All Done " << endl; 
  hf = mfv[0];
  cout << " My first Fish [" <<hf->getName() <<"]"<< endl;
  cleanup_pointer(&mfv);
  cout << " Pointer Cleaned up " << endl; 
}
